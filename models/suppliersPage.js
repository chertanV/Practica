/**
 * suppliersPage.js — страница справочника поставщиков
 * Отвечает за: загрузку списка из Firestore, таблицу, добавление/редактирование/удаление.
 * Поля: fullName, contactPerson, contractNumber, address
 */
import {
  getCollection,
  addDocument,
...
/** Нормализация для сравнения: без учёта регистра и пробелов по краям */
...
/** Сохранение поставщика (создание или обновление). Регистр и пробелы по краям не создают дубликата */
async function saveSupplier() {
  ...
  if (!fullName) {
    alert("Укажите ФИО или название поставщика.");
    return;
  }

  const normalized = normalizeName(fullName);
  const duplicate = suppliersCache.find(
    (s) => s.id !== editingId && normalizeName(s.fullName || s.name) === normalized
  );
  if (duplicate) {
    alert("Поставщик с таким названием уже есть (учитывается без регистра и пробелов в начале/конце).");
    return;
  }
