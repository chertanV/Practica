...
async function saveProduct() {
  ...
  if (editingId) {
    ...
    // Создание: если уже есть товар с тем же штрих-кодом или названием,
    // не создаём новый документ, а обновляем существующий (увеличиваем остаток)
    const barcodeNorm = (barcode || "").trim();
    const nameNorm = name.trim().toLowerCase();
    const existing = productsCache.find((p) => {
      const pBarcodeNorm = (p.barcode || "").trim();
      const pNameNorm = (p.name || "").trim().toLowerCase();
      return (barcodeNorm && pBarcodeNorm === barcodeNorm) || (!barcodeNorm && pNameNorm === nameNorm);
    });
    ...
    reader.onload = async (e) => {
      try {
        ...
        productsSnap.docs.forEach((d) => {
          const data = d.data();
          const b = (data.barcode || "").trim().toLowerCase();
          const n = (data.name || "").trim().toLowerCase();
          if (b) byBarcode[b] = { id: d.id, stock: Number(data.stock ?? 0) };
          if (n) byName[n] = { id: d.id, stock: Number(data.stock ?? 0) };
        });
        ...
        for (const row of rows) {
          let purchasePrice = Number(row.purchasePrice ?? row["Цена закупки"] ?? row.price ?? row.Цена ?? 0);
          let salePrice = Number(row.salePrice ?? row["Цена продажи"] ?? row.price ?? row.Цена ?? 0);
          if (salePrice <= purchasePrice && (purchasePrice > 0 || salePrice > 0)) {
            salePrice = purchasePrice > 0 ? Math.ceil(purchasePrice * 1.1) : salePrice || 0;
          }
          const barcode = String(row.barcode ?? row.Barcode ?? row["Штрих-код"] ?? "").trim();
          const name = (row.name || row.Название || "").trim();
          const existing = (barcode && byBarcode[barcode.toLowerCase()]) || (name && byName[name.toLowerCase()]);

          const importStock = Number(row.stock ?? row.Остаток ?? 0) || 0;

          const baseData = {
            name: name || "",
            supplier: (row.supplier ?? row.Поставщик ?? "").toString().trim(),
        ...
        if (statusEl) statusEl.textContent = `Импорт завершён. Обновлено: ${updated}, добавлено: ${added}.`;
        fileInput.value = "";
        await loadProducts();
        applyFilters();
      } catch (err) {
