 ...
 * Режимы: товары (products) или продажи (sales). Чтение через XLSX, запись в Firestore.
 */
...
/** Парсинг даты из Excel (число, строка или Date). 25569 — смещение для Excel serial date */
...
/** Инициализация: обработка загрузки файла, чтение XLSX, запись в products или sales */
        ...
        for (const row of rows) {
          if (isSales) {
            ...
            const docData = {
              ...
              barcode: String(row.barcode ?? row.Barcode ?? row["Штрих-код"] ?? "").trim(),
