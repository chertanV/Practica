...
/** Отрисовка отчёта в таблице: detail (по товарам) или counterparty (по поставщикам). minSales применяется к агрегированным строкам */
function renderReport(rows, reportType, startDate, endDate, minSales = 0) {
  ...
  if (reportType === "counterparty") {
    ...
    totalTr.innerHTML = `<td colspan="2"><strong>Итого</strong></td><td>${totalQty}</td><td class="price">${totalSum.toLocaleString("ru-RU")}</td>`;
    ...
    if (minSales > 0) aggRows = aggRows.filter((r) => r.quantity >= minSales);
    if (thead) thead.innerHTML = "<th>#</th><th>Название</th><th>Поставщик</th><th>Продажи</th><th>Цена закупки</th><th>Цена продажи</th><th>Выручка</th>";
    tbody.innerHTML = "";
    let totalQty = 0;
    let totalRev = 0;
    aggRows.forEach((r, idx) => {
    ...
    salesSnap.docs.forEach((docSnap) => {
      ...
      if (d.items && Array.isArray(d.items) && d.items.length > 0) {
        d.items.forEach((it) => {
          ...
          const salePrice = it.salePrice ?? product?.salePrice ?? product?.price ?? 0;
          const price = Number(it.discountedPrice ?? it.fullPrice ?? it.price ?? salePrice ?? 0);
          goods.push({
...
/** Генерация отчёта по текущим фильтрам. minSales применяется к агрегату (кол-во проданных по товару/контрагенту) */
